...
/// Strict percent-decoding for raw HTTP query strings, shared by every
/// management API route that filters on a raw `(key, value)` pair list
/// (issue #1151, extracted from the issue #774 `queue-coverage` fix).
