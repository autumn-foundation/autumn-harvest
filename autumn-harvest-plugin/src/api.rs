...
const DEFAULT_PAYLOAD_LIMIT: usize = 2 * 1024 * 1024; // 2MB
...
async fn batch_start_workflows(
    ...
    let Ok(body_bytes) = axum::body::to_bytes(body, max_bytes_usize).await else {
        return (
            StatusCode::PAYLOAD_TOO_LARGE,
            Json(serde_json::json!({
                "error": format!("request body exceeds max_total_bytes ({max_bytes} bytes)")
            })),
        )
            .into_response();
    };
...
async fn query_workflow_post(
    ...
    let body_bytes = axum::body::to_bytes(body, DEFAULT_PAYLOAD_LIMIT)
        .await
        .map_err(|_| AutumnError::bad_request_msg("request body too large"))?;
    let exec_id = parse_execution_id(&id)?;
    let ctx = hydrate_ctx_for_query(&api_state, exec_id).await?;
    let start = Instant::now(); // measure handler invocation latency, not hydration cost
...
async fn bulk_replay_dead_letters_handler(
    ...
    let Ok(body_bytes) = axum::body::to_bytes(body, DEFAULT_PAYLOAD_LIMIT).await else {
        return AutumnError::bad_request_msg("request body too large").into_response();
    };
