...
pub const MAX_API_PAYLOAD_BYTES: usize = 2 * 1024 * 1024; // 2 MiB
...
async fn query_workflow_post(
    ...
    let bytes = axum::body::to_bytes(body, MAX_API_PAYLOAD_BYTES)
        .await
        .map_err(|e| AutumnError::bad_request_msg(format!("payload too large or invalid: {e}")))?;
...
async fn bulk_replay_dead_letters_handler(
    ...
    let bytes = match axum::body::to_bytes(body, MAX_API_PAYLOAD_BYTES).await {
        Ok(b) => b,
        Err(e) => {
            return AutumnError::bad_request_msg(format!("payload too large or invalid: {e}"))
                .into_response();
        }
    };
