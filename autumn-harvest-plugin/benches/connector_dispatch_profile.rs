...
//! broker connector's dispatch pass (issue #944). It drives
//! `ConnectorRuntime::run_once` through `receive -> map -> idempotency key ->
//! dispatch -> decide -> settle` for a realistic batch of inbound messages,
//! against a real Postgres.
//!
//! # Why this workload
//!
//! `autumn-harvest-plugin/src/connector/` is 13k+ lines (`dispatch.rs`,
//! `disposition.rs`, `idempotency.rs`, `binding.rs`, `runtime.rs`, ...). It
//! had no bench, profile, or `docs/performance-*.md` writeup as of this
//! survey. That is unlike almost every other subsystem in this repo.
//!
//! This harness exercises the connector through its real public entry
//! point, `ConnectorRuntime::run_once` -- the same one `run` calls in a
//! loop. It uses `MockSource`, a supported, exported adapter, not
//! test-only scaffolding, so the broker side stays deterministic and
//! network-free. That matches `tests/connector_integration.rs`'s own
//! `soak_ten_thousand_messages_with_redeliveries_and_poison`.
//!
//! Dispatch itself is **not** mocked. Every mapped message goes all the way
//! through `dispatch()` into the real `start_workflow` /
//! `signal_with_start_workflow` handlers and a real Postgres. That is the
//! only way to exercise the actual ack-after-commit and idempotency-dedupe
//! contract this subsystem exists to provide. Mocking dispatch out would
//! profile a different, unrealistic program.
//!
//! # Workload
//!
//! `CONNECTOR_DISPATCH_PROFILE_TOTAL` (default 1,000) Kafka-shaped messages
//! arrive on one partition. The batch matches the soak test's headline
//! *distribution*, not just its total. 1 message in 1,000 is malformed
//! JSON (poison; a small total still rounds up to at least one). 1 message
//! in 20 (5%) is delivered twice at identical coordinates. That forced
//! redelivery stands in for a consumer-group rebalance or an un-acked
//! crash.
//!
//! Every non-poison payload is a realistic order-shaped JSON body
//! (`{"order_id": "...", "customer_id": "...", "items": [...],
//! "total_cents": ...}`), matching `connector_integration.rs`'s
//! `order_mapper` fixture shape. The binding starts a `fulfil_order`
//! workflow, matching that same test file's target.
//!
//! `run_once` runs in a loop, mirroring `ConnectorRuntime::run`'s own
//! driving loop, until a pass receives nothing. That is exactly how the
//! soak test drains its batch.
//!
//! # Running
//!
//! ```text
//! # Needs a reachable Postgres. Point at a local server. A fresh,
//! # uniquely-named database is created, migrated, and dropped for this
//! # run -- never a testcontainer here, so this harness stays runnable
//! # under callgrind on a Docker-less sandbox:
...
//! ```
//!
//! With no reachable database this prints a skip notice and exits 0. That
//! matches `diagnose_bench.rs`'s convention -- never a CI failure.
//!
//! # Wall-clock timing is not admissible evidence
//!
//! This machine is a shared vCPU. Every number this harness's callgrind and
//! dhat runs produce is a deterministic instruction or allocation count,
//! never a wall-clock duration. See `docs/performance-connector-dispatch.md`.
...
/// Total messages the soak-shaped batch pushes before redeliveries. This is
/// scaled down from the soak test's 10,000 so a callgrind run finishes in
/// bounded time. It keeps the distribution (poison, redelivery)
/// proportional.
...
fn with_db_name(admin_url: &str, db: &str) -> Result<String, String> {
    let mut url = reqwest::Url::parse(admin_url).map_err(|e| format!("parse admin url: {e}"))?;
...
/// `valgrind --tool=callgrind`. A Docker-less sandbox is exactly the
/// environment that first needed this profile (see this repo's Bolt survey
/// of the `connector/` subsystem).
async fn setup_bench_db() -> Result<BenchDb, SkipReason> {
    let admin_url = std::env::var("HARVEST_TEST_DATABASE_URL")
        .map_err(|_| SkipReason("HARVEST_TEST_DATABASE_URL is not set".to_string()))?;
    let db = format!(
...
/// database is an operator inconvenience, not a correctness problem. This
/// profiling harness is not expected to run unattended many times a day.
...
/// ...}` -> that order id as the workflow id. This widens the realistic
/// shape `connector_integration.rs`'s `order_mapper` stands in for, with a
/// few more fields. The JSON decode this profile measures is then not a
/// one-field toy.
...
fn seed_batch(source: &MockSource, total: usize) -> usize {
    // `i % poison_every == 0` must fire once per 1,000 messages, not once
    // per `total`. Clamping `total` into `[1, 1000]` gives that period
    // directly and still guarantees at least one poison message when
    // `total` is below 1,000.
    let poison_every = total.clamp(1, 1000);
...
async fn main() {
    ...
    for _ in 0..reps {
        ...
        assert_eq!(
            total_summary.received, delivered,
            "every pushed message was received"
        );
