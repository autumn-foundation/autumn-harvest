...
# form every quickstart/README/CLI doc uses. It only works when Cargo can
...
# `cargo build -p <pkg>` still succeeds (it builds all bin targets), so nothing
# in a build-only CI job catches it. `examples/quickstart` and
# `autumn-harvest-cli` both regressed this way; see the commit that added
# this script for the clean-room reproduction.
#
