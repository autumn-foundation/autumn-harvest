...
#    `edition = "2021"` tutorial setup. The fix is the turbofish rustc
#    itself suggests: `execute_activity::<_, ()>(...)`.
#
...
# workflows). Minimal stand-ins with matching names/signatures let the
# pinned block compile unmodified.
{
  cat <<'RUST_EOF'
...
#[activity]
async fn send_email(_ctx: &ActivityContext, _user_id: i64) -> Result<(), String> {
    Ok(())
}

#[activity]
async fn charge_card(_ctx: &ActivityContext, _user_id: i64) -> Result<(), String> {
    Ok(())
}
