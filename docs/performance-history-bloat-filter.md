...
* **`COUNT(*) >= N` cannot early-exit.** Postgres must count every matching
  ...
  10,000`. Measured: **85,973 total buffers**, a Parallel Seq Scan over the
  whole 2.9M-row table (`pg_stat_statements.total_buffers`, one call).
* **`EXISTS (... ORDER BY event_id OFFSET N-1 LIMIT 1)` is boolean-equivalent**
  ...
  `min(actual_count, N)` rows. Measured on the identical execution: **386
  total buffers** — a **99.55% reduction** — via an Index Only Scan capped at
...
* **The `ORDER BY` is load-bearing, not decorative.** Drop it and the same
  bounded `OFFSET`/`LIMIT` shape still beats the unbounded form (43,166
  buffers vs. 85,973), but the planner falls back to a Seq Scan instead of
  using the existing `(workflow_exec_id, event_id)` index — **111x** worse
  than the indexed form.
* **No regression for the common case.** An ordinary, un-bloated execution
  (5 recorded events) reads the same 4 buffers either way — the filter this
...
DATABASE`. Takes roughly 2-3 minutes — seeding ~2.9M event rows dominates the
runtime.

The fixture is production-shaped, not a toy: 5,000 ordinary `order_flow`
executions with 5-80 events each (a healthy fleet), plus 15 long-running
`poll_loop` executions with 50,000-450,000 events each — the exact "one
workflow accumulated an unbounded history" shape issue #704's
`history_bloat_min_events` filter exists to surface — **2,899,494 events
across 5,015 executions** in total (`fixture-summary.txt`). Both cohorts'
event counts are derived deterministically from `md5(workflow_id)`, so
re-running the harness reproduces the same counts (448,175 events for the
largest bloated execution, every time) and the `pg_stat_statements` buffer
totals below are stable run-to-run — verified across three independent
harness runs, not a one-off.
...
statements captured, the `COUNT(*)` form alone accounts for 85,973 of the
86,359 total buffers touched across both — **99.55% of everything the
...
[**85,973 total buffers**](../perf-artifacts/history-bloat-filter/before-count-star-bloated.explain.txt)
via a Parallel Seq Scan over the entire `harvest_events` table (2,899,494
rows across both cohorts at seed time): Postgres estimated a high-selectivity
equality predicate (`workflow_exec_id = '...'`) cheaper to satisfy by
streaming past the 2.4M+ non-matching rows than by walking the existing
`(workflow_exec_id, event_id)` index. For the 5-event healthy execution the
same form reads
[4 buffers](../perf-artifacts/history-bloat-filter/before-count-star-healthy.explain.txt)
...
[**386 total buffers**](../perf-artifacts/history-bloat-filter/after-bounded-exists-bloated.explain.txt)
(both `pg_stat_statements.total_buffers` and the individual `EXPLAIN
(ANALYZE, BUFFERS)` capture agree exactly) — a **99.55% reduction** — via an
...
[the same 4 buffers](../perf-artifacts/history-bloat-filter/after-bounded-exists-healthy.explain.txt)
...
reads
[**43,166 total buffers**](../perf-artifacts/history-bloat-filter/after-bounded-exists-no-order-by-bloated.explain.txt)
...
found), but via a plain Seq Scan reading 1,434,083 non-matching rows first,
**111x** worse than the `ORDER BY`'d form. The `ORDER BY event_id` clause
...
identical `true`/`false` results between the old and new forms. `N == 0` is
handled as an explicit early return in `apply_min_history_events_filter`
(leaving the query unfiltered, matching `COUNT(*) >= 0`'s always-true
behavior) rather than computing `OFFSET -1`, which Postgres rejects.
...
exercising `OFFSET 6`). These sit alongside the pre-existing 17-test suite
covering AC1/AC2/AC7 for both filters (live/terminal exclusion, sorting,
`history_event_count` reporting, invalid-value rejection, cross-shard
merging, and composition with `state`/pagination/`failure_cause`/
`min_history_events`/`no_progress_minutes`) — all 19 tests pass against the
bounded form with zero behavioral change beyond the buffer-cost fix itself.
