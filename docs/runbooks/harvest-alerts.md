...
4. Read the escalated executions:
   ...
   **What this alert already rules out.** The redelivery budget cannot escalate
   ...
   | 2a | `escalated after R capability-miss redeliveries spread across only D distinct worker(s), hitting the absolute release ceiling of C releases (10x capability_miss_max_redeliveries (N)) …; fewer distinct workers missed this task than the budget allows …` | The task bounced `R` times across **fewer** distinct workers than the budget allows — typically a small (often single-worker) fleet, which can only ever escalate this way. `C` is the computed ceiling (`10 × N`), not `N`. | **This page** | Check those `D` workers first (step 3), *then* the fleet. See the note below. |
