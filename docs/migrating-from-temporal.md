...
| `Promise.race` over MIXED awaitables (activity vs timer, child vs signal, …) | `ctx.race()` with any branch mix; `futures::join!` for the wait-all form | see HVG010 in the determinism guide |
