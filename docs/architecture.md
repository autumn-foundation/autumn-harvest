...
| `payload_codec.rs` | 3.40 | Payload encryption/compression boundary at the event-write path (`PayloadCodec` trait, `PayloadCodecs` registry, `IdentityCodec`; see [ADR-0003](adr/0003-payload-codec-event-boundary.md)). Key rotation (issue #948): `register_key`/`set_active_key`/`active_key_id`/`retire_key_local`, `CODEC_LEGACY_KEY_ID`. The `_harvest_codec_envelope` shape (`codec_envelope_parts`): current writes nest (issue #1253, `CODEC_ENVELOPE_VERSION_NESTED`) once a key is genuinely rotated active or `encode_payload`'s collision-escape guard fires; decode also still reads the two pre-#1253 flat shapes (version 1 legacy / 2 keyed with `kid`) for historical rows. `advertise_codec_capability` (issue #1244) merges this build's `CODEC_ENVELOPE_VERSION_NESTED` support into a worker's `harvest_workers.labels`, consumed by `codec_rotation::activate_codec_key`'s reader-capability handshake. |
