...
- **Background control loops are per-shard.** Timeout/SLA enforcement, poison-pill reclaim, pause auto-resume, session-slot reconciliation, audit export (issue #1269, its own dedicated task since it split off the timeout loop), and the external signal/cancel outboxes each run against every assigned shard.
...
| `payload_codec.rs` | 3.40 | Payload encryption/compression boundary at the event-write path (`PayloadCodec` trait, `PayloadCodecs` registry, `IdentityCodec`; see [ADR-0003](adr/0003-payload-codec-event-boundary.md)). Key rotation (issue #948): `register_key`/`set_active_key`/`active_key_id`/`retire_key_local`, the `_harvest_codec_envelope` shape (`codec_envelope_parts`, versions 1 legacy / 2 keyed with `kid`), `CODEC_LEGACY_KEY_ID`. `advertise_codec_capability` (issue #1244) merges this build's `CODEC_ENVELOPE_VERSION_KEYED` support into a worker's `harvest_workers.labels`, consumed by `codec_rotation::activate_codec_key`'s reader-capability handshake. |
...
| Quota key (#946) | **Re-resolved from the new type's policy**, against the new input — mirrors the concurrency key immediately above |
| `max_input_bytes` (#252, fixed by #1161) | **Re-resolved from the new type.** Enforced at the **worker**, not in `WorkflowContext`: the in-process context cannot see another type's declared cap, so an over-cap input is not rejected synchronously — the worker fails the predecessor terminally at persist time instead. A target declaring no override falls back to the fleet-wide floor, raising it — never lowering it — exactly like every start route |
...
| `throttle` (#607), `debounce` (#499), `batch` (#518) | **Not consulted, by design (#1161).** These *defer or collapse* a start, and continue-as-new is in-flight continuation, not a start — a target declaring one of these gets no pacing when entered via continue-as-new. Gate at the caller instead if a phase must be paced on entry |
| `name`, `module`, `handler` | **Identify the target**, rather than governing successor behavior — `name` becomes `workflow_name`; `handler` is what actually runs the successor |
| `description`, `input_schema`, `output_schema`, `error_schema`, `mcp`, `declared_activities`, `declared_children` | **Not applicable.** Declarative discovery/preflight metadata this path never reads, on any transition |

This partition is exhaustive over every `WorkflowInfo` field (issue #1161): a
newly added field must be placed in one of the tiers above, in both this
...
**Schedule overlap controls now follow a type change (issue #1160).** `schedule_id`/`scheduled_for` are carried (the successor is still the same logical scheduled run); the two overlap controls were fixed to follow that `schedule_id` rather than the schedule's `workflow_name`:
- `OverlapPolicy::CancelOther`/`TerminateOther` (`cancel_in_flight_runs`/`terminate_in_flight_runs`, `scheduler.rs`) select on `schedule_id` alone — no `workflow_name` filter — so they can reach and clean up a cross-type successor. `schedule_id` is unique to one schedule, so dropping the redundant name filter is a no-op for every same-type schedule.
- `max_active_runs` counting (`scheduler::schedule_running_basis`/`schedule_running_basis_batch`) counts `workflow_name = <this schedule's type>` **OR** `schedule_id = <this schedule>` in one `COUNT(*)`, so a row matching both is never double-counted. This is additive, not a re-scope: the name clause alone still covers every same-type run (manual triggers included, proven by a dedicated regression test), and the `schedule_id` clause adds only the cross-type rows the name clause would otherwise miss. Re-scoping the count to `schedule_id` alone was rejected — it would stop counting manual runs of an existing single-type schedule, a behaviour change out of scope for #1160.

A schedule with `OverlapPolicy::Skip` therefore no longer fires again while the previous fire's cross-type successor is still live, and `CancelOther`/`TerminateOther` can now clean that successor up.
...
| `harvest_workers` | `Text` | Live worker process registrations and heartbeat state (`build_id`, `deployment_name` added in issue #171). `labels jsonb` (issue #382) carries operator capability labels for hardware-aware routing, plus one engine-written key: `codec_envelope_version` (issue #1244), merged in by `register_worker`/`heartbeat_worker` on every call via `payload_codec::advertise_codec_capability`. |
...
| `harvest_codec_rotation_cursor` | `Int4` (`shard_id`) | Durable per-shard resume cursor for the lazy payload-codec re-encryption sweep (issue #948). One row per shard: `active_key_id` the pass targets, `last_event_id`/`rows_reencrypted`/`unresolved_rows`, `completed_at` (set only once a full census confirms zero remaining, not merely "reached the end of the batch"). `updated_at` is plain write liveness. `next_revalidation_at` (issue #1258) is a separate throttle deadline for re-censusing an already-completed cursor: null until a pass converges, armed by `write_cursor` on that transition, then rearmed only by `claim_completed_cursor_revalidation` once due — so an ordinary advancing write on a busy shard no longer resets it the way it used to reset `updated_at`. Scoping: shard-local. |
| `harvest_codec_key_state` | `Text` (`key_id`) | Durable fleet-wide codec key lifecycle (issue #1244): `state` ∈ `active`/`retiring`/`retired`, `activated_at`/`retiring_since`/`retired_at`. Partial unique index enforces at most one `active` row. Written by `codec_rotation::activate_codec_key`/`retire_codec_key`; read by every process's scanner tick (`codec_rotation::refresh_active_codec_key`) and by `retire_codec_key`'s structural write-fence gate. Lives on every shard, like `harvest_workers`. |
