...
are implemented. First-class Saga compensation is implemented through the
`Saga` builder.
...
**Terminal workflows (issue #612):** Queries also work against *closed* executions (`COMPLETED`/`FAILED`/`CANCELLED`/`TIMED_OUT`/`TERMINATED`/`CONTINUED_AS_NEW`) for post-mortem state inspection. The engine replays the recorded history and serves the query against the workflow's reconstructed internal state. A run that returned cooperatively (a `COMPLETED` run, or a `FAILED` run whose function returned `Err`) drives all the way to `Poll::Ready`, so the query reads the **final** state — a `FAILED` run therefore answers with its computed internal state, not the error string. A run the engine *sealed while its function was parked mid-command* — `CONTINUED_AS_NEW` (`continue_as_new` parks forever), `TIMED_OUT` (killed on an in-flight activity), or a mid-await external/hard `CANCELLED`/`FAILED` — replays to a *suspension* rather than `Poll::Ready`; because the recorded history still carries the terminal lifecycle event (the "terminal seal"), the query is served against the partial state reconstructed **at that recorded terminal point**. This is read-only — serving a query appends zero events and performs zero writes. Status codes: `200` on success (final or terminal-point state); `404` for an unregistered query name or a fully-deleted execution row (the row itself no longer exists); `410 Gone` "history unavailable" only when the row is present but its history is genuinely unqueryable — pruned by retention, released on reset (no terminal seal in the log), payloads erased, or the replay diverged from the recorded history because the workflow code changed since the run executed (**code drift**: the driven handler settled to a servable state while genuine recorded non-lifecycle history remained unconsumed, so the reconstructed state does not correspond to what actually happened — a truthfully-replayed run leaves only the trailing terminal-lifecycle seal unconsumed, which is excluded, so it still serves `200`); `408` if replay exceeds the configured `query_timeout`. The previous `409` for terminal runs is retired. The `408`/`query_timeout` bound applies to async-yielding replays; a workflow function that busy-loops synchronously without ever `.await`-ing is out of scope, exactly as for the live executor.
...
Setting `harvest.redis.url` needs the `redis` cargo feature of `autumn-harvest-plugin`. A build without that feature carries no channel implementation, so it rejects the URL at load rather than ignoring it. See [`docs/operations/redis-dispatch.md`](operations/redis-dispatch.md).
