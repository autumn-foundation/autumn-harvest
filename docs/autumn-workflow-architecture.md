...
**When Postgres is not enough:** an optional `autumn-harvest-redis` adapter crate exists (see `autumn-harvest-redis/`) that uses Redis Streams for the task queue while keeping Postgres for history and state — an escape hatch, not the default path. Its standalone throughput has been measured (docs/assays/0001-redis-adapter-throughput-ceiling.md), and the founding ">10,000 tasks/second, reliably sustained" claim looks achievable, not refuted: draining a 1,000-row backlog with 8 claimers (the same shape as the Postgres number above) averaged 12,271 claims/sec across three runs, 19.2x the measured Postgres number at matched backlog depth and concurrency. A narrower, artificially-constrained sub-question — an always-near-empty queue at exactly 8 concurrent workers — did miss 10,000/sec (~8,800 mean); see the report for why that is not the same finding. More importantly, **the adapter is not yet wired into the worker** (`autumn-harvest-redis/src/lib.rs` documents the required transactional-boundary refactor as unbuilt), so it cannot be turned on by an operator today regardless of throughput.
