...
matching rows (not the `LIMIT`ed 50), 195 buffers, 11.526ms (see the
timing-noise note below the numbers — this is the largest single
execution-time outlier observed across every rerun of this arm; the
buffer count, 195, has never moved).
...
all 10,000 matching rows, 329 buffers, 4.064ms. Buffers scale
...
seed): no `Sort` node, 53 buffers, bounded scan (50 rows read), 0.078ms.

All four buffer counts are bit-for-bit identical across every rerun of
this apparatus, including a full end-to-end rerun after a post-review fix
(see below) that changed the query text itself and still moved nothing;
execution-time milliseconds are not stable (each arm's own number moved
between runs on identical data — e.g. the control alone read
4.391-5.075ms, and the cardinality-1 arm read as high as 11.526ms in one
run, across otherwise-identical reruns) and are reported only as the
archived run's own figures, never as a claim about relative cost. Buffers,
and the structural presence/absence of a `Sort` node, are what every
verdict in this ledger entry and #6 is graded against.

**Post-review, round 3 (P2, legitimate, fixed in place):** every
diagnostic file in this apparatus, including this record's own
`single_queue_any_diagnostic.sql` and `any_cardinality_2_diagnostic.sql`,
had silently narrowed ledger #5's `SELECT` list (dropping
`concurrency_key`/`concurrency_cap`) and hardcoded `LIMIT 50` instead of
the parameterized `LIMIT :batch_size` the files claimed to reuse
verbatim. Fixed across the whole apparatus (see
[ledger #6](0006-claim-1177-baseline-queue-count.md)'s own post-review
section for the full account) and the apparatus re-run fresh end to end:
every buffer count and every `Sort`-node call in this report is unchanged
by the fix.
