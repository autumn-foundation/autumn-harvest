-- Control: the committed fix's exact concurrency-gate shape (MATERIALIZED
-- CTE + correlated subquery against it), reduced to only the columns/clauses
-- that bear on the gate and the idx_harvest_tq_poll-driven ORDER BY/LIMIT.
