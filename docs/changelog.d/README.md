...
- The file contains the **full changelog entry** that previously went into the phase list: the issue number, what shipped, key design decisions, invariant/append-only notes (new `WorkflowEvent` variant? migration?), and test evidence. Same content, new home.
- **Do NOT edit the phase list in `docs/shipped-work.md` or `CHANGELOG.md` directly in a feature PR.** Those files are now updated only by a periodic collation sweep.

## Collation

A dedicated maintenance session periodically:

1. Folds all accumulated fragments into `docs/shipped-work.md`'s phase list and `CHANGELOG.md`, in one PR.
