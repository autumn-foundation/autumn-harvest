...
existing `delete_unfired_timers_for_execution`) and wires it into
`cancel_and_seal_prior` alongside the existing task/timer cleanup. No new
`WorkflowEvent` variant, no migration — a `harvest_signals` row deletion
