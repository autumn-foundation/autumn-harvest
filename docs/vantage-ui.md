...
- **Cross-site mutation rejection (#1278).** Every `POST` on the management API (Vantage included) rejects a request with no same-origin evidence — a modern browser's `Sec-Fetch-Site: same-origin`, or a matching `Origin`/`Host` pair as a fallback. Neither present means rejected, never admitted by default. `GET`/`HEAD`/`OPTIONS`/`TRACE` and `PUT`/`PATCH`/`DELETE` are exempt (a bare cross-site form or `no-cors` fetch can't send those), and so is a `Content-Type` a form can't send (`application/json`, chiefly) or a request carrying a verified scoped API token. See `same_origin.rs` for the full rationale, including the `X-Forwarded-Proto` scheme check behind a TLS-terminating proxy.
