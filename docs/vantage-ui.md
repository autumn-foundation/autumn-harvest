...
- **Cross-site mutation rejection (#1278).** Every `POST` on the management API, Vantage included, rejects a request with no same-origin evidence — a modern browser's `Sec-Fetch-Site: same-origin`, or a matching `Origin`/`Host` pair as a fallback. Neither header present means rejected; a request is never admitted by default. `GET`/`HEAD`/`OPTIONS`/`TRACE` and `PUT`/`PATCH`/`DELETE` are exempt, since a bare cross-site form or `no-cors` fetch cannot send those. So is a `Content-Type` a form cannot send (`application/json`, chiefly), a request with no `Cookie` header at all (the `harvest` CLI and any other non-browser caller), or a request carrying a verified scoped API token. See `same_origin.rs` for the full rationale, including the `X-Forwarded-Host`/`X-Forwarded-Proto` checks behind a reverse proxy that rewrites `Host` or terminates TLS.
