...
- **Cross-site mutation rejection (#1278).** Every `POST` on the management API, Vantage included, rejects a request with no same-origin evidence — a modern browser's `Sec-Fetch-Site: same-origin`, or a matching `Origin`/`Host` pair (default ports normalized, a reverse proxy's `X-Forwarded-Host`/`X-Forwarded-Proto` preferred when present) as a fallback. Neither header present means rejected; a request is never admitted by default. `GET`/`HEAD`/`OPTIONS`/`TRACE` and `PUT`/`PATCH`/`DELETE` are exempt, since a bare cross-site form or `no-cors` fetch cannot send those. So is a `Content-Type` a form cannot send (`application/json`, chiefly), the `harvest` CLI's own `X-Harvest-Source: cli` header, or a request carrying a verified scoped API token. See `same_origin.rs` for the full rationale, including why a missing session cookie alone is deliberately *not* an exemption — an embedder using browser-cached HTTP Basic/Digest auth would otherwise leak a bypass.
