...
| `doc-snippet-syntax.py` | Every ```` ```rust ```` fenced block in `docs/getting-started/*.md` and `README.md` parses as valid Rust (via `rustfmt`, not `cargo` — a syntax check, not a compile check; see the script's own docstring). A handful of deliberate partial-chain excerpts are documented exceptions, not silently skipped | Yes — `.github/workflows/ci.yml`, `lint` job |
...
| `vantage-flash-live-region.py` | WCAG 4.1.3 status-message live-region role (`role="status"`/`aria-live`) on every Vantage dashboard `div.flash` render site | No — run manually after touching `autumn-harvest-plugin/src/ui.rs`'s flash-rendering sites |
| `vantage-degraded-banner-focus.py` | WCAG 4.1.3 live-region role *and* programmatic on-load focus (`tabindex="-1"` + `autofocus`) on every Vantage dashboard `div.degraded-banner` render site — the `div.flash` mechanism above, extended to the class a full-page-load status message also needs to reach a screen reader on initial content | No — run manually after touching `autumn-harvest-plugin/src/ui.rs`'s degraded-banner-rendering sites |
| `vantage-label-association.py` | Form-label association on every Vantage dashboard `label` render site: the labelled control must resolve, by wrapping or by a `for="…"` matching a real `id="…"` in the file. Scoped to `label` sites — a control with no `label` at all is not separately enumerated | No — run manually after touching `autumn-harvest-plugin/src/ui.rs`'s form-rendering sites |
| `openapi-response-coverage.py` | The API contract against the handlers it describes: every status a handler or the helpers it calls returns — via a literal `StatusCode::` or an `AutumnError::` constructor — is declared for that route, every request-body field that is mandatory on the wire is marked required, every field serde accepts is documented, and every query key a hand-rolled parser matches is documented | Yes — `.github/workflows/ci.yml`, `lint` job |
