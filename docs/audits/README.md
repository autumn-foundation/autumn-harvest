...
| `openapi-response-coverage.py` | The API contract against the handlers it describes: every `StatusCode::` a handler or the helpers it calls returns is declared for that route, and every request-body field that is mandatory on the wire is marked required | Yes — `.github/workflows/ci.yml`, `lint` job |
