...
| `vantage-label-association.py` | Form-label association on every Vantage dashboard `<input>`/`<select>`/`<textarea>`: a `label` must either wrap the control or carry a `for="…"` that resolves to a real `id="…"` in the file | No — run manually after touching `autumn-harvest-plugin/src/ui.rs`'s form-rendering sites |
