...
| `openapi-response-coverage.py` | The API contract against the handlers it describes: every `StatusCode::` a handler or the helpers it calls returns is declared for that route, every request-body field that is mandatory on the wire is marked required, and every field serde accepts is documented | Yes — `.github/workflows/ci.yml`, `lint` job |
