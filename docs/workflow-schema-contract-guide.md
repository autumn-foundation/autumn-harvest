...
| Introducing or changing a **semantic** `format` (`String` → `Uuid`, `uuid` → `date-time`) | A non-numeric `format` is not decoration: `schemars` emits it because the field's Rust type **parses** the string, and that type's `Deserialize` rejects one that does not parse. A recorded arbitrary string stops deserializing the moment the field becomes a `Uuid`. (Numeric formats — `int32`, `double`, … — are orderable and compared on the width lattice instead.) The differ sees only the schema, so a purely descriptive `#[schemars(format = "email")]` on a plain `String` is flagged too — a one-time acknowledgeable cost, taken because the mirror mistake lets a real `String` → `Uuid` break merge silently |
| A **cardinality** bound (`minLength`, `maxLength`, `minItems`, `maxItems`) that is present but is not a non-negative integer | These count things, and the engine's validator reads them with `as_u64`, **ignoring** anything else. So `"minLength": 1.5` is not a *smaller* bound than `1` — it is an *absent* one, and correcting it to `1` newly rejects every recorded empty string. Reading both through `as_f64` would score that a relaxation |
...
| **Duplicating** a `oneOf` branch pinned to a single value | `oneOf` requires exactly one match, so a value matching two identical branches is rejected. (Unit-only enum branches normally collapse into one flat `enum` array so annotation churn produces no delta; that collapse is skipped when it would dedupe a genuine duplicate away, and also when the branches do not **all** agree on `type` — including all agreeing on having none. Disagreeing types have nothing to lift, and collapsing anyway would drop them, making a set and the same set with its types *swapped* — which accepts nothing — canonicalize alike. `anyOf` is "at least one", where duplicates are harmless, so it still collapses) |
...
| **Removing** a semantic `format` (`Uuid` → `String`) | Every recorded value that satisfied the parser is still a perfectly good unconstrained value |
...
There is one deliberate exception to the fail-closed rule, and it is the only
place an unanalysed keyword can change with no delta: a keyword sitting
**beside a `$ref`** that resolves. The engine resolves `$ref` in a loop that
*reassigns* `schema = resolved` and then validates only the landing node, so a
sibling constrains nothing at runtime — editing it cannot reject a recorded
payload, and reporting it would be a false BREAKING rather than a conservative
one. The differ discards siblings the same way, which is what keeps the two in
agreement. This is safe only while **every keyword the engine enforces is one
the differ analyses**, so that invariant is pinned by a test that checks ten
unanalysed draft-07 keywords are genuinely inert at runtime; teaching the engine
one of them fails that test and forces this exception to be revisited. What the
`$ref` *points at* is still compared, so the wrapper is not a blind spot.
...
could move with no delta. Integers compare through `i128`. A **mixed** pair —
one integer, one float, as when a bound is rewritten `9007199254740992.0` →
`9007199254740993` — keeps the integer exact and compares it against the float's
`floor`, with any fractional remainder breaking the tie; converting the integer
to `f64` would reintroduce exactly the collapse above. Only a float/float pair
uses `f64`, where `f64` *is* both values' own representation.
