...
- **It is opt-in, but one cost is not zero.** With no sink configured, no
  ...
  maintenance cost. That cost is bounded only while the audit purge actually
  runs, which needs both `audit_retention_days > 0` and `dry_run` set to
  false. Either condition failing leaves the index growing without bound
  too. Tracked as issue #1272.
