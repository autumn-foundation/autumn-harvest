...
-- populated at birth (100 years in the future, offset by i seconds per
-- row -- far enough out that it never elapses no matter how long this
-- script's drain takes to run; matches the Rust harness's seeded value
-- exactly -- see the note on `SCHEDULE_TO_CLOSE_SQL` in
-- claim_budget_tests.rs for why 'infinity' was tried and rejected, and why
-- a per-row offset matters: a byte-identical value across all rows lets
-- Postgres's B-tree deduplication compress the index far more than
-- production's genuinely distinct per-row deadlines would, understating
-- real index growth -- Codex review on PR #1339), then the identical
-- claim-shaped per-row drain -- still never touching schedule_to_close_at,
-- but each new tuple version must still carry its 8 extra heap bytes AND a
