...
- **Same-name resolution is scope-aware.** When two first-party helpers share a name, a bare call is resolved using Rust-accurate nearest-scope preference: a **globally unique** helper (the only definition anywhere) resolves directly, else a helper in the caller's **own module** wins. Every other case — multiple same-named candidates, none in the caller's own module (e.g. the real target is a `use`-imported helper from another module or file, which this text-level analysis cannot follow) — is conservatively skipped (a safe false-negative). A same-file-but-different-module helper is deliberately **not** preferred: a bare call in a child module does not reach a parent- or sibling-module helper without a `use`, so resolving to an in-file same-named helper would be a false positive. This adds no false-positive risk.
- **Local shadowing (source-order aware).** A call whose name is bound by the caller — a fn-pointer / closure **parameter**, a local **closure** binding, or a shadowing `let` — is treated as the local, not a same-named free helper. `#[workflow(fn ...)]` fn-pointer params and closures are exactly the forms #386 excludes. Suppression is **source-order aware**: a `let` shadow only suppresses a call on a strictly-later source line, so a call that runs *before* any same-name binding still resolves to (and is scanned as) the free helper, matching Rust (a call on the binding's own line, e.g. `let bad = bad();`, resolves to the free helper too). Params bind for the whole body. Residual safe false-negatives: a `let` shadow bound in an inner block that has already closed before an outer call still suppresses that call (source-order, not full brace-scope tracking), and if a body both shadows a name *and* legitimately calls a real free fn of that name after the shadow, that call is conservatively skipped.
- **Path-qualified one-hop calls.** `self::helper()`, `crate::mod::helper()`, and `super::helper()` are **not** resolved — only bare-ident calls (`helper()`, including a turbofish `helper::<T>()`) are matched. A `use`-imported call resolves only when the name is **globally unique**; an ambiguous import (several same-named helpers, real target elsewhere) is conservatively skipped rather than resolved to a wrong in-file candidate (see "Same-name resolution" above). Resolving qualified / associated-function (`Type::assoc()`) calls would risk associated-function/method false positives, which is exactly the #386 boundary.
