...
- **Same-name resolution is scope-aware.** When two first-party helpers share a name, a bare call is resolved using Rust-accurate nearest-scope preference: a helper in the caller's **own module** wins, else the sole helper in the caller's **file**, else a **globally unique** helper. Only the genuinely-undisambiguable case — multiple candidates, none in the caller's module or file scope — is conservatively skipped (a safe false-negative). Preferring the same-module / same-file definition is exactly what the compiler resolves a bare call to, so this adds no false-positive risk.
...
- **Path-qualified one-hop calls.** `self::helper()`, `crate::mod::helper()`, and `super::helper()` are **not** resolved — only bare-ident calls (`helper()`, including a turbofish `helper::<T>()`) and `use`-imported calls are. Resolving qualified / associated-function (`Type::assoc()`) calls would risk associated-function/method false positives, which is exactly the #386 boundary.
