...
| `METRIC_ACTIVITY_PAUSE_ACTIONS` | `harvest.activity.pause_actions` | Counter | `activity` (= `METRIC_LABEL_ACTIVITY`, bounded **by bucketing** — the pause routes accept an unregistered name by design, so the emitter resolves the raw name against the registered activity catalogue and substitutes `UNREGISTERED_ACTIVITY_NAME` (`__unregistered__`) when absent; `validate_activity_name` bounds length, not cardinality), `action` (2 values: pause/resume, bounded by `ActivityPauseAction`) — one operator pause/resume action on a whole activity type (issue #807), counted only when it genuinely changed state (`newly_paused`/`newly_resumed`), so an idempotent retry is not a second hold. A counter, not a gauge: it answers "how often is this lever pulled, and on what", not "is a hold in effect" (read `GET /activities` for the state). Named `pause_actions` rather than `paused` because `harvest.queue.paused` (Gauge, issue #619) and `harvest.workflow.paused` (Counter, issue #383) already disagree on instrument type under that noun | `execution.id` |
...
| `METRIC_RATE_LIMIT_BUCKETS_DELETED` | `harvest.retention.rate_limit_buckets_deleted` | Counter | `family` (2 values: `dyn-rate`/`start-throttle`, bounded) — inert per-tenant rate-limit buckets collected by the idle-bucket GC (issue #1127) | the bucket `key` (unbounded: one value per tenant, which is exactly why these rows need collecting), `execution.id` |
| `METRIC_SCANNER_TICK`      | `harvest.scanner.tick`        | Counter      | `scanner` (bounded, 8 values: timeout/sla/poison_pill/external_outbox/retention/schedule/pause_auto_resume/audit_export; 8 labels but 6 spawned loops — `sla`/`external_outbox` are ticked by the `timeout` loop that drives them, so those three cannot diverge), `shard` (operator-configured shard id, or `none` for the process-wide `retention`/`schedule` loops and single-shard deployments — required so a healthy per-shard sibling cannot mask a wedged one on a shared series) | `execution.id` |
