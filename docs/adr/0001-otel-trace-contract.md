...
| `METRIC_WORKFLOW_HISTORY_BLOAT` | `harvest.workflow.history_bloat` | Counter | `workflow` (= `METRIC_LABEL_WORKFLOW`, bounded) — fires exactly once per still-**RUNNING** (non-terminal) execution when its recorded `harvest_events` count first crosses `history_bloat_warn_fraction * event_hard_cap` (`WorkflowHistoryPolicy`, issue #704, default fraction `0.75`). Observation-only early warning: the run keeps executing normally; a no-op (permanently flat series) for any workflow type with no `event_hard_cap` configured. Distinct from `harvest.workflow.history_oversized` (a periodically-**sampled gauge** driven by the separate, fleet-wide `HarvestBuilder::max_workflow_history_events` ceiling, issue #493) | `execution.id` |
