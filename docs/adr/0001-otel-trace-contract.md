...
| `METRIC_SIGNAL_UNHANDLED`  | `harvest.signal.unhandled`    | Counter      | `workflow` (= `METRIC_LABEL_WORKFLOW`), `name` (signal name, bounded), `queue` — **graceful terminals only** (`Completed`/`Failed` reached through the workflow drive); forced-failure / scanner terminal paths (`TIMED_OUT`/`CANCELLED`/`TERMINATED`/parent-close cascade/history-cap failure) have no driven matcher and are NOT counted | `execution.id` |
