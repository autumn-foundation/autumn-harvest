...
| `METRIC_SIGNAL_RECEIVED`   | `harvest.signal.received`     | Counter      | `workflow` (= `METRIC_LABEL_WORKFLOW`), `name` (= `METRIC_LABEL_NAME`, signal name, bounded), `queue` | `execution.id` |
| `METRIC_SIGNAL_UNHANDLED`  | `harvest.signal.unhandled`    | Counter      | `workflow` (= `METRIC_LABEL_WORKFLOW`), `name` (signal name, bounded), `queue` — **graceful terminals only** (`Completed`/`Failed`); `TIMED_OUT`/`CANCELLED`/`TERMINATED`/parent-close have no driven matcher and are NOT counted | `execution.id` |
| `METRIC_UPDATE_ADMITTED`   | `harvest.update.admitted`     | Counter      | `workflow` (= `METRIC_LABEL_WORKFLOW`), `name` (= `METRIC_LABEL_NAME`, update name, bounded) | `execution.id` |
