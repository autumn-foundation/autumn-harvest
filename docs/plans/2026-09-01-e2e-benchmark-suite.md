...
|---|---|---|
| R1 | Publish a p99 computed from three samples. | Every scenario carries a minimum sample count; below it the report renders `n/a` and the run is marked unsound rather than printing a confident-looking number. Test: `stats_below_the_minimum_sample_count_are_unsound`. |
...
| R10 | Publish numbers whose hardware/config is undocumented. | The report renders the environment block (CPU count, OS/arch, profile, worker and pool configuration, and Postgres `version()` read from the live server) and the results file is a copy of that output rather than a hand-typed table. The worker/concurrency configuration is additionally published on `docs/benchmarks.md` and pinned there by `the_doc_publishes_the_worker_configuration_the_numbers_were_taken_at`. |
| R11 | Let the doc's numbers and the harness's baselines drift. | Baselines are constants. `benchmarks_docs.rs` fails when the versioned results file lacks a matching **row** (scenario, shard count *and* value — not just the number somewhere in the file), when the index page lacks the number, when the documented command is not the committed runner, when a documented environment variable is not one the harness reads, or when the shard matrix, tolerance or worker configuration in prose disagrees with the code. |
| R12 | Quietly turn this into a CI gate that flakes the build. | No manifest row runs any DB scenario. The only wired rows are the pure unit tests and the docs-drift guard. Test: `no_ci_manifest_row_runs_the_end_to_end_benchmark`. |
