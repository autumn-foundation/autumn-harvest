...
| Should core grow a `StorageBackend` trait? | **No.** Buildable, but costed below at a scale the benefit does not justify — 24 of 54 coupled modules are portable only by dropping a capability or reimplementing wholesale, for a use case that does not share the Postgres concurrency model. |
...
|---|---|---|
| `diesel` query layer | 54 modules | Query construction is mechanical; the *type* layer is not. |
...
**54 of the 108 core modules** exhibit at least one mechanism — a shade under
...
That 38 of the 54 coupled modules hand-write SQL is therefore the more
...
| Component | Scope |
|---|---|
| Trait definition + Postgres impl | ~54 modules touched |
...
- The companion crate delivered that capability at **structurally zero cost** to
  the Postgres path, against a seam sized above at ~54 modules touched.
