...
| Should core grow a `StorageBackend` trait? | **No.** Buildable, but costed below at a scale the benefit does not justify — 24 of 53 coupled modules are portable only by dropping a capability or reimplementing wholesale, for a use case that does not share the Postgres concurrency model. |
...
|---|---|---|
| `diesel` query layer | 53 modules | Query construction is mechanical; the *type* layer is not. |
...
| `raw-sql` — reaches for Diesel's raw-SQL escape hatch (`sql::<…>`, `sql_query`) | 37 modules | Case by case — the SQL must be read, not inferred from the ORM. |
...
**53 of the 108 core modules** exhibit at least one mechanism — a shade under
...
That 37 of the 53 coupled modules hand-write SQL is therefore the more
...
| Component | Scope |
|---|---|
| Trait definition + Postgres impl | ~53 modules touched |
...
- **24 modules are class (c)** — portable only by dropping a capability or
  reimplementing wholesale — against 6 that are trivially trait-able.
- **37 modules reach for raw SQL**, so their portability cannot be read off
...
- The companion crate delivered that capability at **structurally zero cost** to
  the Postgres path, against a seam sized above at ~53 modules touched.
