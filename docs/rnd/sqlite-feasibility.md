...
| `raw-sql` — reaches for Diesel's raw-SQL escape hatch (`sql::<…>`, `sql_query`) | 34 modules | Case by case — the SQL must be read, not inferred from the ORM. |
