...
Plus **101 migrations** written in Postgres DDL (`JSONB`, `TIMESTAMPTZ`,
...
| `quota_reconcile` | diesel, raw-sql | (b) | Periodic backfill sweep for pre-upgrade `quota_key` rows (issue #1226). Plain DML -- a `SELECT ... LIMIT $1` and one `UPDATE ... WHERE id = $2 AND quota_key IS NULL` -- with nothing Postgres-specific in it, so the substitute is a direct transcription; it is (b) rather than (a) only because the SQL is hand-written and must be read rather than inferred from the ORM. No advisory lock: the `WHERE quota_key IS NULL` predicate on both the scan and the write is itself sufficient to make two concurrent sweeps safe, so there is nothing here for a single write lock to subsume. |
