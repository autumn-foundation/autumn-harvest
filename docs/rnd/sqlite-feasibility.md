...
sqlite-spike,testing --test integration sqlite_spike` → **20 passed; 0 failed**
...
| `scenario_ctx_info_workflow_type_is_cross_backend_faithful` | **(Codex P2 regression, round 9 — AC4 fidelity)** A workflow reads the replay-safe `ctx.info().workflow_type` and both returns it and feeds it as the activity input. Under the SQLite runtime it observes the **real** registered handler name (`echo_workflow_type`), NOT `""`, and the resulting history replays clean on the engine's own `WorkflowReplayer` — which threads `workflow_name` from the history snapshot — so the two backends **agree** on the recorded `ActivityScheduled.input`. Fails on the pre-fix code where `drive_one_cycle` reconstructed the `WorkflowContext` via the thin `run_workflow_with_state` (which hardcodes an empty `workflow_name`): the SQLite backend observed `""` and recorded `""` into the activity input, so a PG-shaped import would diverge against the engine's recomputed real name. The fix threads the stored `workflow_name` into the full executor entry point (`run_workflow_with_state_history_policy_and_caps`) so `ctx.info().workflow_type` / `ctx.workflow_type()` are cross-backend-faithful. |
