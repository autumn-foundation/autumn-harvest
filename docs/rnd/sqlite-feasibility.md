...
| `listen/notify` push wakeups | 5 modules | No — polling is a degradation, not a translation. |
...
| `chaos` | diesel, listen/notify | (c) | Deterministic fault injection (issue #940). The injection points are portable; the reproducers wake on `LISTEN/NOTIFY`, which polling can only approximate. |
