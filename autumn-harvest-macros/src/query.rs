...
//
// Issue #1632 (Echo findings) documents `build_query_dispatch`'s arity-keyed
...
// module for the sibling async sites.
#[cfg(test)]
mod dispatch_characterization_tests {
    use super::query_macro;
    use quote::quote;

    /// Isolate `__dispatch`'s body, the block between the sync fn's `Result`
    /// return type and the enclosing companion fn's closing brace. Mirrors
    /// `workflow.rs`'s `extract_dispatch_body`, adjusted for query's
    /// non-`async` signature (no `Box::pin(async move { ... })` wrapper).
    fn extract_dispatch_body(full: &str) -> String {
        let marker = ":: autumn_harvest :: serde_json :: Value , String > {";
        let start = full
            .find(marker)
