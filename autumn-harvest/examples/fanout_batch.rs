...
/// # Errors
///
/// Returns an error if the item string starts with `"bad_"`.
...
/// # Errors
///
/// This mock activity never returns an error, but the signature supports it.
...
/// # Errors
///
/// Returns an error if any of the fanned-out `process_item` activities fail.
#[workflow]
...
/// # Errors
///
/// Returns an error if the engine fails to schedule or execute the fan-out step
/// (e.g. payload too large), though individual activity failures are collected.
#[workflow]
...
/// # Errors
///
/// Returns an error if the `fetch_items` activity fails or if the engine
/// fails to schedule the fan-out collection.
#[workflow]
