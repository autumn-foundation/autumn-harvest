...
//! weighted_queue_order}` -- the weighted-random queue-selection step
...
//! not just the queues an operator bothered to assign an explicit weight to.
//! This harness reproduces exactly that pair of calls, over a realistic
//! multi-queue binding: a handful of explicitly weighted queues (the
//! operator's actual incident-response/prioritization intent), the rest
//! defaulting to weight 1, and one weight-0 "backfill" queue (drained only
//! when every positive-weight queue is empty) -- the shape
//! `queue_fairness.rs`'s own doc comment describes.
//!
...
//! how many poll iterations are simulated -- roughly 500 seconds of
//! continuous back-to-back polling at the documented 25ms poll interval
//! (`docs/benchmarks.md`), the regime a saturated queue actually runs under
//! (a worker claiming tasks back to back never waits at all, per that page).
//! The queue list and weight map are built ONCE, outside the measured loop,
//! isolating the two functions' own cost from setup.
//!
...
/// queue is pinned to weight 0 (a backfill queue), and the rest fall back to
...
fn main() {
    ...
    for _ in 0..reps {
        ...
        // Self-checked so a change that silently dropped or duplicated a
        // queue (not just its cost) would fail this harness rather than
        // produce a quietly-wrong "faster" number.
        assert_eq!(
            ordered.len(),
            n,
