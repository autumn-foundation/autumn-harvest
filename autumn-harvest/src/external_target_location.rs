...
pub async fn resolve_location_by_workflow_id_with(
    ...
    let (candidates, uninspected) =
        match fan_out_candidates(pool, router, workflow_name, workflow_id, held, memo).await {
            Ok(pair) => pair,
...
/// touching `conn` again. See the inline comment on that arm below. `Ok`
/// carries every candidate the fan-out found and every shard it could not
/// inspect, for either reduction to use.
async fn fan_out_candidates(
    ...
    memo: Option<&UninspectableShards>,
) -> Result<(Vec<(ShardId, ResolvedRun)>, Vec<UninspectedShard>), Vec<UninspectedShard>> {
...
pub async fn check_cross_shard_occupancy(
    ...
    match fan_out_candidates(pool, router, workflow_name, workflow_id, held, None).await {
        Ok((candidates, uninspected)) => {
