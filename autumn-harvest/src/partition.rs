...
/// sequence behind [`DROP_GATE_INDEX_MARKER`]) so the probe step can refuse
/// to reuse a same-named relation it did not create.
///
/// `CREATE SEQUENCE` on a fixed name, unguarded, silently succeeds against
/// an operator's own unrelated sequence of that name. A later step would
/// then `DROP` it. The probe step checks this comment before touching an
/// existing relation, and only ever creates the sequence itself when the
/// name is free.
...
pub fn enable_sql(opts: &EnableOptions) -> String {
    ...
    // Re-checked under the lock below, not just by `enable_partitioning`'s
    // own pre-flight calls (`refuse_if_row_security` and friends). Those
    // run in an earlier, separate statement. A schema change landing in
    // the gap between that statement and this one passed no check at all.
    //
    // Omitted entirely when unwanted, like the scripted plan's own
    // phase-1 and phase-4 guards, rather than left in and only
    // conditionally skipped. An operator who opted out of the
    // publication guard must not still be refused here.
    let recheck_publication = if opts.allow_incompatible_publications {
        String::new()
    } else {
        "    SELECT string_agg(DISTINCT t.pubname, ', ') INTO bad\n      \
             ...
             the pre-flight check ran. The partitioned layout is not compatible with a flat \
             logical-replication subscriber. Run the partitioned layout on the subscriber \
             too, or drop the publication, then re-run this.', bad;\n    \
             END IF;\n"
            .to_string()
    };
    format!(
...
BEGIN
    ...
    -- and friends) ran in an earlier, separate statement, before this
    -- script started. A schema change landing in that gap passed no check
    -- at all. `ALTER TABLE ... RENAME` below acquires this same lock
    -- anyway, so this adds no new wait; it only closes that gap.
    EXECUTE 'LOCK TABLE harvest_events IN ACCESS EXCLUSIVE MODE';

{recheck_publication}    SELECT COALESCE(bool_or(c.relrowsecurity OR c.relforcerowsecurity), false)
      INTO rls
      FROM pg_class c
      JOIN pg_namespace n ON n.oid = c.relnamespace
     WHERE c.relname = 'harvest_events' AND n.nspname = current_schema();
...
(policies: %), added after the pre-flight check ran. Drop the policies, then re-run this.',
            COALESCE(pols, 'none, but row security is enabled');
    END IF;
...
the pre-flight check ran. Drop or redefine the view, then re-run this.', bad;
    END IF;
...
engine (%), created after the pre-flight check ran. Drop the trigger, then re-run this.', bad;
    ...
    IF bad IS NOT NULL THEN
        RAISE EXCEPTION 'harvest #958: unique index(es) on harvest_events do not include \
cohort (%), created after the pre-flight check ran. Drop or redefine the index, then \
re-run this.', bad;
    END IF;
    ...
    -- Probed only AFTER the rename. A `SELECT EXISTS` before it takes only
    -- ACCESS SHARE, which does not conflict with a concurrent INSERT: a
    -- workflow that started
...
pub async fn disable_partitioning(
    ...
    let report = Box::pin(
        conn.transaction::<DisableReport, HarvestError, _>(async |conn| {
            ...
            // Taken explicitly, and re-checked immediately after, rather
            // than left to whichever later statement happens to need the
            // lock. The orphan/duplicate deletes above can run long. A
            // view, trigger or policy landing on `harvest_events` during
            // that window passed the guards above with nothing to catch
            // it. `ALTER TABLE ... RENAME` below acquires this same lock
            // anyway, so taking it here first adds no new wait. It only
            // closes the gap between a stale check and the destructive
            // `DROP TABLE ... CASCADE` further down.
...
pub fn migration_plan_steps(opts: &EnableOptions, now: DateTime<Utc>) -> Vec<PlanStep> {
    ...
    let mut steps = vec![
        ...
        //
        // The reading itself lives in a `COMMENT`, not a row, on a
        // `SEQUENCE` this step creates for exactly that purpose — not a
        // `TABLE`. Postgres structurally excludes sequences from every
        // publication, `FOR ALL TABLES` included: `CREATE PUBLICATION`
        // only ever admits ordinary and partitioned tables. A zero-column
        // TABLE still passed that filter. It could be discovered as a
        // sync target by a `CREATE SUBSCRIPTION` or `REFRESH PUBLICATION`
        // landing while it existed, whether or not it ever held a row. A
        // sequence has no such exposure, by construction, not by staying
        // empty. Four properties fall out of that choice, each behind its
        // own guard below:
        //
        // - Ownership. `CREATE SEQUENCE` on a fixed name, unguarded, would
        //   silently reuse an unrelated operator sequence of that exact
        //   name. The mark step would then `DROP` it. A pre-existing
        //   relation is only ever reused when its own comment already
        //   carries `PROBE_TABLE_MARKER`. Any other relation there aborts
        //   the step instead of touching it.
        //
        ...
        //   ever written once; a second pass over an already-commented
        //   sequence leaves it alone.
        //
        // - Crash durability. A comment is catalog metadata, WAL logged
        //   like any other catalog change. It survives a crash between
        //   the build and the mark step exactly as durably as the
        //   engine-created index it is tracking. An `UNLOGGED` table
        //   would also dodge publication membership, but it loses its
        //   state on crash recovery.
        //
        // - Publication safety, structurally rather than incidentally.
        //   See above — this is the property the choice of `SEQUENCE`
        //   over `TABLE` exists for.
        step(
            2,
            format!(
                "DO $harvest_dropgate_probe_958$\nDECLARE marker text;\nBEGIN\n    \
                 ...
                 END IF;\n    \
                 ELSE\n        \
                 CREATE SEQUENCE harvest_1270_drop_gate_probe;\n    \
                 END IF;\n    \
                 SELECT obj_description('harvest_1270_drop_gate_probe'::regclass, 'pg_class')\n      \
                 INTO marker;\n    \
                 IF marker IS NULL THEN\n        \
                 EXECUTE 'COMMENT ON SEQUENCE harvest_1270_drop_gate_probe IS '\n            \
        ...
        step(
            ...
            format!(
                "DO $harvest_dropgate_mark_958$\nDECLARE marker text;\nBEGIN\n    \
                 ...
                 || quote_literal('{DROP_GATE_INDEX_MARKER}');\n    \
                 END IF;\n    \
                 DROP SEQUENCE IF EXISTS harvest_1270_drop_gate_probe;\n\
