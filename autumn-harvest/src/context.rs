...
impl WorkflowContext {
    ...
    pub fn execute_query_with_args(&self, name: &str, args: Value) -> HarvestResult<Value> {
        ...
        if let Some(h) = decl_handler {
            return std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| h(args)))
                .map_err(|e| {
                    let msg = e
                        .downcast_ref::<&str>()
                        .map(|s| (*s).to_string())
    ...
    pub fn register_declarative_update_handler(&self, info: &crate::info::UpdateHandlerInfo) {
        ...
        // Also wire the validator into the imperative update registry so that
        // validate_update works for declarative handlers.
        if info.has_validator {
            if let Some(validator_fn) = info.validator {
                let handler_fn = info.handler;
                let name = info.name;
                let boxed_validator: crate::update::BoxUpdateValidator =
                    std::sync::Arc::new(validator_fn);
                let boxed_handler: crate::update::BoxUpdateHandler =
                    std::sync::Arc::new(move |input| Box::pin(handler_fn(input)));
    ...
    pub fn invoke_update(
        ...
        // Fall back to the declarative-only map (not yet wired into the imperative
        // registry, e.g., handlers registered without going through
        // register_declarative_update_handler).
        let handler = self
            .declarative_updates
            .lock()
            .expect("declarative_updates lock poisoned")
