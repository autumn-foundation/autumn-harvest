...
impl WorkflowContext {
    ...
    ///
    /// The returned context exposes `state::<T>()` but otherwise behaves as an
    /// empty live context — scheduling activities or signals from inside an
    /// update handler is not supported.
    #[must_use]
    pub fn new_for_handler(state: SharedState) -> std::sync::Arc<Self> {
    ...
    pub fn register_declarative_update_handler(&self, info: &crate::info::UpdateHandlerInfo) {
        ...
        // WorkflowContext (with shared state) on each invocation. This keeps the
        // closure 'static while still giving the user function access to
        // ctx.state::<T>() at dispatch time.
        ...
        let boxed_handler: crate::update::BoxUpdateHandler =
            std::sync::Arc::new(move |input| {
                let ctx = Self::new_for_handler(std::sync::Arc::clone(&state));
    ...
    pub fn invoke_update(
        ...
        let handler = self
            ...
            .copied();
        handler.map(|h| {
            let ctx = Self::new_for_handler(std::sync::Arc::clone(&self.state));
            h(ctx, input)
        })
    }
