...
pub struct WorkerConfigView {
    ...
    /// registered for the task's type) before a task escalates to the
    /// terminal-failure / DLQ path with a `no_capable_worker:` reason
    /// (issue #804). `0` escalates on the first miss.
