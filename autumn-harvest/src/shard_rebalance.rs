...
mod db {
    ...
    /// pointer is usually collapsed straight to the live shard (one hop), but
    /// the walk still confirms with one more read past it, and that
    ...
    pub async fn resolve_execution_shard_holding(
        ...
        let mut current = origin;
        for hop in 0..MAX_FORWARD_HOPS {
            let forward = if on_held(current) {
                read_forward(conn, exec_id).await?
            } else {
                let mut hop_conn = if hop == 0 && !forwarded {
                    checkout_entry(pool, current).await?
                } else {
                    checkout(pool, current).await?
                };
