...
mod tests {
    ...
    #[tokio::test]
    async fn test_next_event_id_overflow() {
        // We will construct a scenario where next_event_id is very high (e.g. i32::MAX),
        // and check that incrementing it with checked_add behaves correctly.
        let next_event_id = i32::MAX;

        let result = next_event_id
            .checked_add(1)
            .ok_or_else(|| crate::error::HarvestError::Database("Event ID overflow".to_string()));
        ...
        assert!(matches!(
            result.unwrap_err(),
            crate::error::HarvestError::Database(_)
        ));
    }
}
