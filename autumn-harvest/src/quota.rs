...
/// Resolve [`QuotaPolicy::key_expr`] for one admission, tolerating a
/// batched-start aggregate array (issue #1230 Finding 1).
///
/// `harvest_event_batches` merges every admission's payload into one JSON
/// array before it fires (`event_batch.rs`'s `buffered_payloads`). The
/// merged array reaches this call as the fired execution's `input`.
/// [`resolve_quota_key`] requires an object at the first path segment. It
/// returns `None` for an array input. That silently skips all three quota
/// dimensions for every batched execution, regardless of the declared cap.
///
/// This function resolves against the array's FIRST element instead of
/// against the array itself. The batch's quota charge is attributed to
/// whichever admission arrived first for the batch key. That mirrors the
/// first-admission-wins rule `harvest_event_batches` already applies to
/// every other captured start option (see `event_batch.rs`'s upsert SQL
/// comment). A batch whose members resolve to different keys still
/// charges only the first. That is a documented approximation, not a bug:
/// a merged execution has exactly one quota charge, and the alternative
/// (today's behavior) charges nothing at all.
///
/// A non-array input resolves exactly as [`resolve_quota_key`] resolves
/// it. This function changes behavior only for the array case. A direct
/// (non-batched) start observes no change.
///
/// # Examples
///
/// ```rust
/// use autumn_harvest::quota::resolve_quota_key_for_admission_input;
///
/// let object = serde_json::json!({ "tenant_id": "acme" });
/// assert_eq!(
///     resolve_quota_key_for_admission_input("tenant_id", &object),
///     Some("acme".to_string()),
/// );
///
/// let batched = serde_json::json!([{ "tenant_id": "acme" }, { "tenant_id": "other" }]);
/// assert_eq!(
///     resolve_quota_key_for_admission_input("tenant_id", &batched),
///     Some("acme".to_string()),
/// );
/// ```
#[must_use]
pub fn resolve_quota_key_for_admission_input(
    expr: &str,
    ...
    input
        ...
        .map_or_else(
            || resolve_quota_key(expr, input),
            |first| resolve_quota_key(expr, first),
        )
}
...
mod tests {
    ...
    // -- resolve_quota_key_for_admission_input (issue #1230 Finding 1) ------

    #[test]
    fn resolve_quota_key_for_admission_input_matches_plain_resolver_for_an_object() {
        // A direct (non-batched) start's input is an object. Behavior must
        // be byte-identical to `resolve_quota_key` -- this function changes
        // resolution only for the array case.
        let input = serde_json::json!({ "tenant_id": "acme" });
        assert_eq!(
            resolve_quota_key_for_admission_input("tenant_id", &input),
            resolve_quota_key("tenant_id", &input),
        );
        assert_eq!(
            resolve_quota_key_for_admission_input("tenant_id", &input),
    ...
    #[test]
    fn resolve_quota_key_for_admission_input_resolves_a_batched_array_via_first_element() {
        // The exact shape `event_batch.rs` fires with: every buffered
        // admission's payload merged into one array. Before this function
        // existed, `resolve_quota_key` returned `None` here every time --
        // issue #1230 Finding 1's silent, complete quota bypass.
    ...
    #[test]
    fn resolve_quota_key_for_admission_input_plain_resolver_still_returns_none_for_an_array() {
        // Documents the exact bug this function fixes. The resolver it
        // delegates to for the non-array case is unchanged. It still
        // fails closed on an array top level.
        let batched = serde_json::json!([{ "tenant_id": "acme" }]);
        assert_eq!(resolve_quota_key("tenant_id", &batched), None);
    }

    #[test]
    fn resolve_quota_key_for_admission_input_empty_array_is_none() {
        let empty = serde_json::json!([]);
        assert_eq!(
            resolve_quota_key_for_admission_input("tenant_id", &empty),
            None
        );
    }

    #[test]
    fn resolve_quota_key_for_admission_input_first_element_missing_field_is_none() {
        let batched = serde_json::json!([{ "other": "value" }, { "tenant_id": "acme" }]);
        assert_eq!(
            resolve_quota_key_for_admission_input("tenant_id", &batched),
            None,
            "the FIRST element decides the batch's key -- a later element's \
             value must not be picked up as a fallback"
        );
    }

    #[test]
    fn resolve_quota_key_for_admission_input_first_element_not_an_object_is_none() {
        let batched = serde_json::json!(["not an object", { "tenant_id": "acme" }]);
        assert_eq!(
            resolve_quota_key_for_admission_input("tenant_id", &batched),
            None
        );
