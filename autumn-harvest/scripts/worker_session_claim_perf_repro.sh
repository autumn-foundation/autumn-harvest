#!/usr/bin/env bash
# Reproduction harness for the worker-sessions claim-predicate measurement
# (issue #606 / Ledger perf pass) documented in
# `docs/performance-worker-sessions.md`.
#
...
# `claim_budget_tests::zz_capture_worker_session_claim_evidence` ONCE. That
# single test drives the real `claim_task_query()`/`claim_task()` at TWO
# seeded *data* states of `harvest_task_queue.session_id` /
# `harvest_task_queue.sticky_worker_id` -- both `NULL` (today's default) vs.
# every row pinned to the claiming worker's own session -- and captures both,
# so there is nothing else for this script to toggle.
#
...
#
# Writes into `docs/perf-artifacts/worker-session-claim-predicate/`:
#   {no-session,worker-session}-claim-backlog-{1000,10000,100000}.explain.txt
#     `EXPLAIN (ANALYZE, BUFFERS, VERBOSE, SETTINGS, TIMING OFF)` for
#     `claim_task_query()` at each published `BACKLOG_SWEEP` depth, at both
#     seeded states.
#   {no-session,worker-session}-pg_stat_statements.txt
...
OUT_DIR="$REPO_ROOT/docs/perf-artifacts/worker-session-claim-predicate"
TEST_FILTER="claim_budget_tests::zz_capture_worker_session_claim_evidence"
...
LOG="/tmp/worker_session_claim_capture.log"
...
if ! grep -q "^== capture complete: worker-session evidence, artifacts in " "$LOG"; then
