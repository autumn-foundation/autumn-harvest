#!/usr/bin/env bash
# Reproduction harness for the concurrency-key claim-gate rewrite in
...
#                      (`running_by_key AS MATERIALIZED`), and materialize
#                      that commit's content into the working file for the
...
#
# Writes into `docs/perf-artifacts/concurrency-key-claim-gate/`:
#   {before,after}-claim-backlog-{1000,10000,100000}.explain.txt
#     `EXPLAIN (ANALYZE, BUFFERS, VERBOSE, SETTINGS, TIMING OFF)` for
#     `claim_task_query()` at each published `BACKLOG_SWEEP` depth, with
#     `ClaimGate::ConcurrencyKey` seeded (256 distinct keys, cap=1_000_000 so
#     the gate never blocks -- isolates the predicate's evaluation cost from
#     any change in which rows are eligible).
...
#     backlog under the same gate.
#   {before,after}-fixture-summary.txt
#     Seeded/claimable row counts per depth.
#
...
OUT_DIR="$REPO_ROOT/docs/perf-artifacts/concurrency-key-claim-gate"
TEST_FILTER="claim_budget_tests::zz_capture_concurrency_key_claim_evidence"
FIX_MARKER="running_by_key AS MATERIALIZED"
