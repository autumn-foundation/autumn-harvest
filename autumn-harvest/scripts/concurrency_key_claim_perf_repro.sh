#!/usr/bin/env bash
# Reproduction harness for the per-key concurrency gate rewrite in
# `queue::claim_task_query()` (issue #247 / Ledger perf pass) documented in
# `docs/performance.md`.
#
# Runs the `#[ignore]`d evidence-capture test
# `claim_budget_tests::zz_capture_concurrency_key_claim_evidence` TWICE
# against the SAME harness, fixture generator, and (where the harness falls
# back to a testcontainer) the same freshly-created database within one
# script invocation:
#
...
#                      (`concurrency_pending_keys AS MATERIALIZED`), and
#                      materialize that commit's content into the working
#                      file for the duration of the "before" capture. Either
#                      way the original working-tree file is restored on
#                      exit (success, failure, or interrupt).
#
...
#
# Writes into `docs/perf-artifacts/concurrency-key-claim-predicate/`:
#   {before,after}-claim-backlog-{1000,10000,100000}.explain.txt
#     `EXPLAIN (ANALYZE, BUFFERS, VERBOSE, SETTINGS, TIMING OFF)` for
#     `claim_task_query()` at each published `BACKLOG_SWEEP` depth, under the
#     standard idle (non-blocking-cap) `ClaimGate::ConcurrencyKey` scenario --
#     directly corroborates `docs/performance.md`'s existing +644% published
#     number, since that number was itself measured under this exact seed
#     shape.
#   {before,after}-claim-backlog-10000-hot-contention.explain.txt
#     The same EXPLAIN, but with 2,000 RUNNING rows seeded across the SAME
#     concurrency keys the 10k-row PENDING backlog uses -- this predicate's
#     cost is load-dependent (unlike the queue-pause anti-join), so this
#     capture demonstrates the worse-than-idle real-world case.
...
#     backlog under the idle concurrency-key scenario.
#   {before,after}-fixture-summary.txt
#     Seeded/claimable row counts per depth, plus the hot-contention and
#     stat-snapshot fixture shapes.
#
...
OUT_DIR="$REPO_ROOT/docs/perf-artifacts/concurrency-key-claim-predicate"
TEST_FILTER="claim_budget_tests::zz_capture_concurrency_key_claim_evidence"
FIX_MARKER="concurrency_pending_keys AS MATERIALIZED"
...
capture() {
  ...
  cargo test -p autumn-harvest --features db,testing --test integration -- \
    --ignored --nocapture "$TEST_FILTER" 2>&1 | tee "/tmp/concurrency_key_claim_capture_${label}.log"

  local produced
  produced="$(grep -c "^== capture complete: label=${label}," "/tmp/concurrency_key_claim_capture_${label}.log" || true)"
...
failed/skipped. Check the log at /tmp/concurrency_key_claim_capture_${label}.log." >&2
    exit 1
  fi
}
...
${content}" >&2
    ...
    # finds a match, closing its read end -- on content this size (~170KB)
