#!/usr/bin/env bash
# Reproduction harness for the capability-labels claim-predicate measurement
# (issue #382 / Ledger perf pass) documented in
# `docs/performance-capability-labels.md`.
#
# Unlike `queue_pause_claim_perf_repro.sh`, this does NOT toggle a code fix:
# `queue::claim_task_query()` is unmodified end to end -- measurement found no
# query-shape fix that helps (see the doc page for why). It runs the
# `#[ignore]`d evidence-capture test
# `claim_budget_tests::zz_capture_capability_labels_claim_evidence` ONCE. That
# single test drives the real `claim_task_query()`/`claim_task()` at TWO
# seeded *data* states of `harvest_task_queue.required_capabilities` --
# `NULL` (today's default) vs. every row carrying one `Exact` requirement the
# claiming worker's `labels` are made to satisfy -- and captures both, so
# there is nothing else for this script to toggle.
#
...
#
# Writes into `docs/perf-artifacts/capability-labels-claim-predicate/`:
#   {no-capabilities,capability-labels}-claim-backlog-{1000,10000,100000}.explain.txt
#     `EXPLAIN (ANALYZE, BUFFERS, VERBOSE, SETTINGS, TIMING OFF)` for
#     `claim_task_query()` at each published `BACKLOG_SWEEP` depth, at both
#     seeded states.
#   {no-capabilities,capability-labels}-pg_stat_statements.txt
...
OUT_DIR="$REPO_ROOT/docs/perf-artifacts/capability-labels-claim-predicate"
TEST_FILTER="claim_budget_tests::zz_capture_capability_labels_claim_evidence"
...
LOG="/tmp/capability_labels_claim_capture.log"
...
if ! grep -q "^== capture complete: capability-labels evidence, artifacts in " "$LOG"; then
