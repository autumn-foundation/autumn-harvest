...
-- members by (workflow_name, workflow_id, shard_id) — the shard_id filter scopes
-- the gather to the queried row's logical shard (multiple logical shards can
-- share one physical database, and CAN chains are shard-local). The only
-- existing index on the name/id pair is the PARTIAL uniqueness index
...
-- query loads. Add a full (non-partial) index covering the shard_id predicate so
-- that gather is an index scan instead of a sequential scan on large legacy tables.
CREATE INDEX IF NOT EXISTS idx_harvest_wfx_workflow_identity
    ON harvest_workflow_executions (workflow_name, workflow_id, shard_id);
