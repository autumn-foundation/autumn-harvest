...
use autumn_harvest::admission_gate::{
    AdmissionGate, AdmissionGateId, GateScope, check_admission,
};
...
fn make_expiring_gate(scope: GateScope, reason: &str, expires: chrono::DateTime<Utc>) -> AdmissionGate {
    AdmissionGate {
        id: AdmissionGateId(Uuid::new_v4()),
        scope,
        reason: reason.to_string(),
...
fn workflow_name_scope_matches_specific_workflow() {
    let gates = [make_gate(GateScope::WorkflowName("payment_flow".to_string()), "blocked")];
    let result = check_admission(&gates, "payment_flow", "default", 0, None);
    assert!(result.is_some(), "WorkflowName gate must block the named workflow");
}
...
fn workflow_name_scope_does_not_match_other_workflows() {
    ...
    assert!(result.is_none(), "WorkflowName gate must not block other workflows");
}

#[test]
fn queue_scope_matches_specific_queue() {
    let gates = [make_gate(GateScope::Queue("email-workers".to_string()), "outage")];
    let result = check_admission(&gates, "send_emails", "email-workers", 0, None);
    assert!(result.is_some(), "Queue gate must block work on the named queue");
}
...
fn shard_scope_matches_specific_shard() {
    ...
    assert!(result.is_some(), "ShardId gate must block work on the named shard");
}
...
fn owner_scope_matches_specific_owner() {
    let gates = [make_gate(GateScope::Owner("team-billing".to_string()), "billing down")];
    let result = check_admission(&gates, "billing_reconcile", "default", 0, Some("team-billing"));
...
fn owner_scope_does_not_match_other_owners() {
    let gates = [make_gate(GateScope::Owner("team-billing".to_string()), "billing down")];
    let result = check_admission(&gates, "send_email", "default", 0, Some("team-notifications"));
...
fn owner_scope_does_not_match_no_owner() {
    ...
    assert!(result.is_none(), "Owner gate must not block workflows with no owner");
}
...
fn expired_gate_does_not_block() {
    ...
    let gates = [make_expiring_gate(GateScope::Fleet, "expired incident", past)];
    let result = check_admission(&gates, "any_workflow", "default", 0, None);
...
fn future_expiry_gate_blocks() {
    ...
    let gates = [make_expiring_gate(GateScope::Fleet, "live incident", future)];
    let result = check_admission(&gates, "any_workflow", "default", 0, None);
...
fn no_expiry_gate_blocks_indefinitely() {
    ...
    let result = check_admission(&gates, "any_workflow", "default", 0, None);
    assert!(result.is_some(), "Gate with no expiry must block admissions");
}
...
fn no_gates_never_blocks() {
    ...
    assert!(result.is_none(), "Empty gate list must not block admissions");
}
...
fn first_matching_gate_is_returned() {
    ...
    assert_eq!(matched.reason, first_reason, "First matching gate should be returned");
}
...
fn or_semantics_any_matching_gate_blocks() {
    ...
    assert!(result.is_some(), "Fleet gate must still block even when queue-scoped gate misses");
}
...
fn admission_blocked_error_displays_correctly() {
    ...
    assert!(msg.contains("admission blocked"), "Error display must mention 'admission blocked'");
    assert!(msg.contains("prod incident"), "Error display must include the reason");
}
...
fn max_active_gates_constant_is_documented() {
    ...
    assert!(autumn_harvest::admission_gate::MAX_ACTIVE_GATES >= 10,
        "MAX_ACTIVE_GATES must allow at least 10 simultaneous gates");
    assert!(autumn_harvest::admission_gate::MAX_ACTIVE_GATES <= 1000,
