...
// regenerated by build.rs from the whole `migrations/` directory. Replaced
// a hand-rolled `concat!(include_str!(...))` list that had drifted (it was
// missing `harvest_execution_summaries` and `harvest_external_task_updated_at`,
// so tests failed `relation ... does not exist` / hit a stale uniqueness
// index) — invisible because this suite was never run against a DB in CI
