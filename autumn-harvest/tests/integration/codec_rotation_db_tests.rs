...
#[tokio::test]
async fn an_escaped_row_is_neither_counted_nor_swept() {
    // issue #1253: `encode_payload`'s write-time escape guard wraps business
    // data that collides with the envelope shape in a `_harvest_codec_escaped`
    // marker. The census SQL matches on `_harvest_codec_envelope`. That key
    // is absent from the wrapper's own top level. So an escaped row must be
    // exactly as invisible to the census and sweep as any other near-envelope.
    use autumn_harvest::schema::harvest_events;

    let (url, _c) = setup_isolated_db().await;
    let mut conn = connect(&url).await;
    let codecs = two_key_registry();
    let exec_id = insert_execution(&mut conn, "escaped").await;
    append_under_key(
        &mut conn,
        &codecs,
        exec_id,
    ...
    let mut data: Value = harvest_events::table
        ...
        .await
        .expect("row");
    // The exact colliding shape an identity-codec deployment escapes at write
    // time (issue #1253's reported collision).
    let colliding_business_data = json!({
        "_harvest_codec_envelope": 2,
        "codec_id": "xor",
        "data": "AAAA",
        "kid": "2026-q3",
    });
    let escaped = PayloadCodecs::default()
        .encode_payload(&colliding_business_data)
        .expect("encode");
    assert_ne!(
        escaped, colliding_business_data,
        "sanity: the colliding shape must have been escaped"
    );
    data["data"]["input"] = escaped;
    diesel::update(harvest_events::table.find(row_id))
        .set(harvest_events::event_data.eq(&data))
        .execute(&mut conn)
        .await
    ...
    assert_eq!(
        progress.rows_remaining(),
        0,
        "an escaped row must not be counted by the census"
    );
    assert_eq!(
        sweep_codec_reencryption_once(&mut conn, 0, &codecs, 100, &NoOpMetrics)
            .await
