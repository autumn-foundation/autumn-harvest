...
fn changelog_fragment_names_the_index_cost() {
    ...
    assert!(
        names_index_cost_near(&text, "partial index"),
        "{}: the opt-in bullet must name the partial index, its maintenance \
         cost, and issue #1272 all within reach of each other",
        path.display()
    );
    assert!(
