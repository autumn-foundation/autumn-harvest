...
use autumn_harvest_verify::analysis::summary::Analyzer;
...
// ── Issue #1296 P1: the fixpoint cap needs a boundary on exhaustion ─────────

/// A body of `hops` blocks, laid out in **reverse dependency order**. A
/// flow-insensitive round sweeps blocks in file order, so it propagates
/// the source's taint only one hop per round. `bb0` reads `_1` before
/// `bb1` (which writes `_1` from `_2`) runs in that same round, and so on
/// down the chain. `bb{hops-1}` is the only source (`SystemTime::now`);
/// `bb0` writes the return place `_0`. Reaching `_0` therefore takes on
/// the order of `hops` rounds, deliberately more than `MAX_ROUNDS` (24)
/// when `hops` is large enough. That is exactly the shape the fixpoint
/// cap has to give up on honestly rather than silently.
fn reverse_chain_body(hops: usize) -> String {
    use std::fmt::Write as _;

    assert!(hops >= 2, "need at least one intermediate hop");
    let mut locals = String::new();
    for local in 1..hops {
        let _ = writeln!(locals, "    let _{local}: u64;");
    }
    let mut blocks = String::new();
    blocks.push_str("    bb0: {\n        _0 = copy _1;\n        return;\n    }\n\n");
    for block in 1..hops - 1 {
        let _ = writeln!(
            blocks,
            "    bb{block}: {{\n        _{block} = copy _{};\n        goto -> bb{};\n    }}\n",
            block + 1,
    ...
    let _ = writeln!(
        blocks,
        "    bb{last}: {{\n        _{last} = SystemTime::now() -> [return: bb{}, unwind continue];\n    }}",
        last - 1
    );
    format!("fn wf_long_chain() -> u64 {{\n    let mut _0: u64;\n{locals}\n{blocks}}}\n")
}

/// The boundaries `Analyzer::analyze_body` collects for a body generated by
/// [`reverse_chain_body`].
fn chain_boundaries(hops: usize) -> Vec<autumn_harvest_verify::verdict::Boundary> {
    let text = reverse_chain_body(hops);
...
#[test]
fn a_short_chain_converges_within_the_round_cap() {
    let boundaries = chain_boundaries(10);
    assert!(
        !boundaries
            .iter()
            .any(|b| b.kind == BoundaryKind::FixpointExhausted),
        "a 9-hop chain must converge well within 24 rounds: {boundaries:?}"
    );
}

#[test]
fn an_unconverged_fixpoint_is_a_named_boundary_never_a_silent_partial_answer() {
    // 44 hops, laid out so a round only advances the taint one hop — far past
    // `MAX_ROUNDS` (24).
    let boundaries = chain_boundaries(45);
    assert!(
        boundaries
            .iter()
            .any(|b| b.kind == BoundaryKind::FixpointExhausted),
        "a body whose taint fixpoint does not converge within MAX_ROUNDS must \
         raise `fixpoint-exhausted`, not report as if it had converged; \
         boundaries = {boundaries:?}"
    );
}

// ── Issue #1296 P1: sanitizer kills must respect statement order ───────────

/// One function, `read_first`-controlled: the sink reads the
/// `HashMap::keys()` collection either **before** or **after** `sort()`,
/// by swapping which of them runs in `bb2` vs `bb3`. A method call is
/// always a MIR block *terminator*. The sink and the sort therefore never
/// share a block — block order alone settles which side of the sanitizer
/// the read is on.
fn order_body(name: &str, read_first: bool) -> String {
    ...
    format!(
        ...
        \x20   }}\n\n\
        \x20   bb2: {{\n\
        \x20       {first} -> [return: bb3, unwind continue];\n\
        \x20   }}\n\n\
        \x20   bb3: {{\n\
        \x20       {second} -> [return: bb4, unwind continue];\n\
        \x20   }}\n\n\
        \x20   bb4: {{\n\
        \x20       return;\n\
        \x20   }}\n}}\n"
...
fn order_findings(name: &str, read_first: bool) -> Vec<Finding> {
    let text = order_body(name, read_first);
    let doc = mir::parse("fixture", "generated.mir", &text);
    let program = Program::build(vec![doc], &SourceRoots::default()).expect("build");
    let m = model();
    let mut analyzer = Analyzer::new(&program, &m);
...
#[test]
fn a_value_read_before_the_sanitizer_keeps_its_taint() {
    let findings = order_findings("wf_read_before_sort", true);
    assert!(
        !findings.is_empty(),
        "the sink reads the HashMap keys before `sort()` ever runs; the kill \
         must not reach backwards to a read that executes first"
    );
}

#[test]
fn a_value_read_after_the_sanitizer_is_clean() {
    let findings = order_findings("wf_sort_before_read", false);
    assert!(
        findings.is_empty(),
...
// ── Issue #1296 P1: body-less function-item callbacks are classified ───────

/// Runs `body_text`'s `wf_*` fn through the analyzer directly (no entry
/// discovery needed) and returns the boundaries and findings it collected.
fn analyze_generated(body_text: &str, path: &str) -> (Vec<Boundary>, Vec<Finding>) {
    let doc = mir::parse("fixture", "generated.mir", body_text);
    let program = Program::build(vec![doc], &SourceRoots::default()).expect("build");
    let m = model();
    let mut analyzer = Analyzer::new(&program, &m);
    analyzer.analyze_body(
    ...
    (analyzer.boundaries, analyzer.findings)
}

#[test]
fn a_body_less_function_item_callback_starts_a_source() {
    // `.unwrap_or_else(SystemTime::now)`: the callback has no body in the
    // analyzed set, but its callee path is a `[[source]]` row. It must be
    // classified exactly as a direct call to it would be.
    let text = "fn wf_fn_item_source(_1: &WorkflowContext, _2: u64) -> u64 {\n\
        \x20   let mut _0: u64;\n\
        \x20   let _3: u64;\n\n\
        \x20   bb0: {\n\
        \x20       _3 = Option::<u64>::unwrap_or_else(move _2, const SystemTime::now) -> [return: bb1, unwind continue];\n\
        \x20   }\n\n\
        \x20   bb1: {\n\
        ...
        \x20       return;\n\
        \x20   }\n}\n";
    let (boundaries, findings) = analyze_generated(text, "wf_fn_item_source");
    assert!(
        !findings.is_empty(),
        "SystemTime::now via a bare fn-item callback must start a source, \
         same as a direct call; boundaries = {boundaries:?}"
    );
}

#[test]
fn a_trusted_body_less_function_item_callback_stays_clean() {
    // `.map(String::from)`: the callback is body-less but its owner path is
    // std-rooted, so it must stay a trusted propagator — never a boundary,
    // never a source.
    let text = "fn wf_fn_item_clean(_1: &WorkflowContext, _2: std::option::Option<u32>) -> u64 {\n\
        \x20   let mut _0: u64;\n\
        \x20   let _3: std::string::String;\n\n\
        \x20   bb0: {\n\
        \x20       _3 = Option::<u32>::map(move _2, const std::string::String::from) -> [return: bb1, unwind continue];\n\
        \x20   }\n\n\
        \x20   bb1: {\n\
        \x20       _0 = WorkflowContext::execute_activity_raw(move _1, const 0_u64) -> [return: bb2, unwind continue];\n\
        \x20   }\n\n\
        \x20   bb2: {\n\
        \x20       return;\n\
        \x20   }\n}\n";
    let (boundaries, findings) = analyze_generated(text, "wf_fn_item_clean");
    assert!(
        boundaries.is_empty() && findings.is_empty(),
        "a std::String::from callback must stay a clean propagator: \
         boundaries = {boundaries:?}, findings = {findings:?}"
    );
}

// ── Issue #1296 P1: an unresolved closure argument is a named boundary ─────

#[test]
fn a_closure_argument_with_no_body_in_the_analyzed_set_is_a_boundary() {
    // A closure span is present at the call site, but no body anywhere in
    // the doc set was printed with it. That happens with a partial dump,
    // or a registration whose body never made it into the emitted MIR.
    // Silently contributing nothing here is how a callback with a sink or
    // a source inside it went unseen.
    let text = "fn wf_missing_closure(_1: &WorkflowContext) -> u64 {\n\
        \x20   let mut _0: u64;\n\
        \x20   let _2: {closure@fake.rs:1:1: 1:2};\n\
        \x20   let _3: u64;\n\n\
        \x20   bb0: {\n\
        \x20       _3 = call_with_closure(move _2) -> [return: bb1, unwind continue];\n\
        \x20   }\n\n\
        \x20   bb1: {\n\
        \x20       return;\n\
        \x20   }\n}\n";
    let (boundaries, _findings) = analyze_generated(text, "wf_missing_closure");
    assert!(
        boundaries
            .iter()
