...
//!
//! RED phase. `Program::build` is `todo!()`, and the resolution *query* API does
//! not exist yet at all — every test that needs it is spelled out, with the
//! exact proposed signatures, in the commented-out `mod api_gap` at the bottom
//! so this file still compiles against today's scaffold.
...
//     );
//     assert_eq!(
//         program.resolve_call("wf::{closure#0}", "<{async fn body of sub()} as Future>::poll"),
//         Resolution::Body("sub::{closure#0}".to_string())
//     );
// }
//
...
//         Resolution::Boundary(BoundaryKind::DynDispatch, "<dyn Src as Src>::next".to_string())
//     );
// }
//
// #[test]
...
//     let program =
//         Program::build(vec![parse_fixture("format_and_outparams.mir")], &fixture_roots()).unwrap();
//     assert_eq!(
//         program.resolve_call("wf_dyn_two_impls::{closure#0}", "<dyn Namer as Namer>::name"),
//         Resolution::Boundary(BoundaryKind::DynDispatch, "<dyn Namer as Namer>::name".to_string()),
...
//         Program::build(vec![parse_fixture("format_and_outparams.mir")], &fixture_roots()).unwrap();
//     assert_eq!(
//         program.resolve_call("wf_dyn_single_impl::{closure#0}", "<dyn Clock as Clock>::now_secs"),
...
//     assert!(
//         matches!(resolution, Resolution::Boundary(BoundaryKind::IndirectCall, _)),
//         "got {resolution:?}"
//     );
// }
//
...
//     let program = Program::build(vec![parse_fixture("generic_layers.mir")], &fixture_roots()).unwrap();
...
//         vec!["inner::<Wrapper<Leaf>>".to_string(), "inner::<Wrapper<Leaf>>".to_string()]
//     );
...
//         Resolution::Boundary(BoundaryKind::ExternalCrateBody, "some_crate::never_emitted".to_string())
//     );
// }
//
// #[test]
...
//     let program =
//         Program::build(vec![parse_fixture("example_deterministic_primitives.mir")], &workspace_roots())
//             .unwrap();
//     assert!(program.body("notify_decision::{closure#0}").is_some());
//     assert!(program.body_paths().contains(&"__autumn_workflow_info_notify_decision"));
// }
