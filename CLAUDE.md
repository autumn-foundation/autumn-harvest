...
async fn subscription(ctx: &WorkflowContext, tier: String) -> Result<String, String> {
    ...
    ctx.register_signal_handler("cancel", move |req: CancelRequest| {
        *state.lock().unwrap() = true;
        tracing::info!(reason = %req.reason, "subscription cancelled");
    });

    // Untyped variant for dynamic dispatch:
    ctx.register_signal_handler_raw("pause", |payload: serde_json::Value| {
        tracing::info!(?payload, "subscription paused");
    });
...
**Mechanics.** `WorkflowContext::register_signal_handler` (typed, `Req: Deserialize`) and `register_signal_handler_raw` (untyped `serde_json::Value`) store the handler in an in-memory `SignalHandlerRegistry` (`signal_handler.rs`) and, in the same call, drain every currently-recorded `SignalReceived` event for that name via `HistoryMatcher::drain_signal_events` — mirroring `drain_admitted_updates`: a full scan of history independent of the main replay cursor, so a handler registered before any other cursor-based scan has run this cycle still finds everything already recorded for its name. **No new `WorkflowEvent` variant** — `SignalReceived` is reused, so the append-only invariant and adjacently-tagged JSON contract are untouched. Handlers are **fire-and-forget** and **synchronous**: no validator, no completion event, no suspension shape to reason about — a mutation of captured state is visible to the rest of the workflow body immediately, within the same cycle. A panicking handler is caught and logged rather than crashing the workflow task; a payload that fails to deserialize under the typed variant is logged and dropped.
