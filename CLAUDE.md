...
| Cancellation propagation | Activity cancellation | `ParentClosePolicy` (issue #347) governs what happens to still-running children when the parent closes |

**Replay-safety, determinism, and cancellation are byte-identical to activity fan-out** — the feature reuses the same mechanism end-to-end rather than introducing a parallel contract:
...
- **Cancellation**: both methods check `ctx.is_cancelled()` before scheduling and return `HarvestError::Cancelled`. Cancellation after dispatch propagates to in-flight children per the existing `ParentClosePolicy` semantics (issue #347) — unchanged from single-child spawning.
