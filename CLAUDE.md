...
- **All-or-nothing dispatch**: on a fresh (first-time) fan-out, every child's serialized input is checked against `payload_max_workflow_input` *before* any child is dispatched — so an oversized input in slot N cannot leave slots `0..N` mid-dispatch (command pushed, `ExecutionId` allocated) while the fan-out call fails outright due to `try_join_all`'s poll-order-dependent short-circuit (fixed post-review). The fan-out now deterministically dispatches all N children or none. This does **not** by itself change what gets persisted on a terminal failure — the worker's terminal-failure path never turns `StartChildWorkflow` commands into events regardless (a pre-existing, broader limitation shared by any workflow that starts a child then fails, not specific to fan-out) — but it does mean the *in-memory* attempt is always clean (no wasted per-child dispatch work, no poll-order-dependent partial state) and the failure surfaces immediately with an unambiguous reason.
